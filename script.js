// ===== Elements =====
// Safety guards (avoid console errors if an element is missing)
if (!yesBtn || !noBtn || !note || !modal || !modalTitle || !modalText || !modalOk || !modalCopy || !bgMusic || !musicBtn || !musicLabel || !volume || !soundOverlay || !soundOverlayBtn) {
  console.warn("Valentine page: missing required elements. Check your HTML IDs.");
}
...
// iOS sometimes fires change instead of input
...
// Keep the "No" button inside the button area (mobile-friendly)
...
function dodgeNo() {
  ...
  // Make it absolute only while dodging
  ...
  const maxLeft = Math.max(pad, area.width - btn.width - pad);
  const maxTop  = Math.max(pad, area.height - btn.height - pad);

  const left = randomWithin(pad, maxLeft);
  const top  = randomWithin(pad, maxTop);
...
// Desktop + mobile pointer behavior
noBtn.addEventListener("pointerenter", dodgeNo);

// Mobile tap: if still dodging, move instead of click
